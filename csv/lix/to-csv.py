...
def main():
    srcPth   = Path("bruttostichprobe-wave-2.dta")
    mixinPth = srcPth.with_name("mixin-wave-2.csv")
    ...
    try:
        ...
        df1["email"] = df1["email"].where(df1["email"].notna(), "").astype(str).str.replace("info@geruestbau.heil.de.", "info@geruestbau.heil.de", regex=False)
